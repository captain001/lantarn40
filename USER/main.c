...
//¶¨ÒåÍâ²¿±äÁ¿
extern struct AreaStruct area[8]; //·ÖÇøÊôĞÔ½á¹¹
extern struct ScrStruct  scr_info;//ÆÁÊôĞÔ½á¹¹
extern unsigned char pro_num,info_num,pro_c; //½ÚÄ¿×ÜÊı¡¢ĞÅÏ¢¸öÊı¡¢ĞÅÏ¢¼ÆÊıÆ÷
extern unsigned char sim_buf_red[];//»º³åÇø£¬´æ·Å´ÓÄÚ²¿flashÖĞ¶Á³öµÄÊı¾İ
...
extern unsigned char wifi_init_flag,wifi_config,wifiinit,timer1_flag,config_step;                    //wifi ³õÊ¼»¯±êÖ¾Î» //ÉÏµçÆô¶¯ 0£»ÅäÖÃ 1£»Õı³£¹¤×÷ 2£»
extern unsigned char screeniniterr;

extern	unsigned char		mm_set_en;		//=1ÔÊĞíÉèÖÃWIFIÃÜÂë
...
** º¯ÊıÃû³Æ £º delay
** º¯Êı¹¦ÄÜ £º ÑÓÊ±º¯Êı  
** Èë¿Ú²ÎÊı £º ĞèÒªÑÓÊ±µÄÊ±¼ä
** ·µ»Ø²ÎÊı £º ÎŞ
** º¯ÊıËµÃ÷ £º 
** ĞŞ¸ÄÈÕÖ¾ £º 2013.3.18 ÖìÕñÍş	
**************************************************************************/
...
/**************************************************************************
** º¯ÊıÃû³Æ £º PowerManagerInit
** º¯Êı¹¦ÄÜ £º µçÔ´¹ÜÀí³õÊ¼»¯ 
** Èë¿Ú²ÎÊı £º ÎŞ
** ·µ»Ø²ÎÊı £º ÎŞ
** º¯ÊıËµÃ÷ £º 
** ĞŞ¸ÄÈÕÖ¾ £º 2013.4.06 ÖìÕñÍş	
**************************************************************************/
static void PowerManagerInit(void)
{
	#define rPWR_CR  (*(volatile unsigned long *)0x40007000)
	
	/* ¸ù¾İSTM32Êı¾İÊÖ²áµÚ44Ò³Éè¶¨µçÑ¹·§Öµ£¬ÕâÀïÊÇ2.9V */
	rPWR_CR |= (7 << 5); 
	/* Æô¶¯PWR¹¦ÄÜ */
	rPWR_CR |= (1 << 4);
}
...
/**************************************************************************
** º¯ÊıÃû³Æ £º PowerManagerInit
** º¯Êı¹¦ÄÜ £º µÍÑ¹¼ì²âÖĞ¶Ï
** Èë¿Ú²ÎÊı £º ÎŞ
** ·µ»Ø²ÎÊı £º ÎŞ
** º¯ÊıËµÃ÷ £º µçÑ¹Ô½¹ı·§Öµ£¨2.9£©·üÊ±»á½øÈë´ËÖĞ¶Ï
** ĞŞ¸ÄÈÕÖ¾ £º 2013.4.06 ÖìÕñÍş	
**************************************************************************/
...
//	unsigned char		indi;
	...
	clr_screen();					//ÇåÆÁ=================================================================
	NVIC_Configuration(); //ÖĞ¶Ï¿ØÖÆÆ÷³õÊ¼»¯		
	PowerManagerInit();   //µçÔ´¹ÜÀí³õÊ¼»¯ 2013.04.06Ôö¼Ó
	IWDG_Init(7,4000);    	//Óë·ÖÆµÊıÎª64,ÖØÔØÖµÎª625,Òç³öÊ±¼äÎª25s   
 ...
 uart2_init(115200);
   ...
   init_finish=0x55;
	...
	screeniniterr=0;
		...
		ch734init();//³õÊ¼»¯²¢¶ÁÈ¡Êı¾İ
	#endif
	spi_init(0x30f);//spi ³õÊ¼»¯£¬4·ÖÆµ
		
 	//flash_check();//¼ì²âflashÖĞÓĞÎŞÊı¾İ		=========================================================
 	screen_init();//ÆÁĞÅÏ¢³õÊ¼»¯
	timer3_init();//¶¨Ê±Æ÷3³õÊ¼»¯
	clr_screen();//ÇåÆÁ
/************************×Ô¼ì*********12.11.15******************************/
	if (!(GPIOA->IDR & (1 << 13))) 
	{		   //Èç¹ûGPA13¹Ü½ÅÎªµÍµçÆ½   °´ÏÂ
		...
		if ((GPIOA->IDR & (1 << 13)) && (2 == ucCh))
		{
			...
			if ((GPIOA->IDR & (1 << 13)) && (2 == ucCh))
			{
				...
				if(flash_read(0)!=0xff){ //ÅĞ¶ÏÊÇ·ñ²Á³ıÍê
						flash_byte_write(0x0);
						flash_chip_erase(); //ÔÙ´Î²Á³ı
				}
			...
			while (g_wShowVersionCnt < 300)//2013.5.30
			{
				...
				#ifdef	WIFI_CARD_8266
					flash_read_write(1);
			  sim_buf_red[0x55] = 0xab;//0xab; //Ïû³ı¸´Î»ÖØĞÂÅäÖÆ
			  flash_read_write(0);
...
/**********************************************************************************/
  exti_init(); //°´¼üÍâ²¿ÖĞ¶Ï³õÊ¼»¯	
	...
	light+=2;								//×î´óÁÁ¶È½µµÍÁ½¸öµµ
	
// 	if(sim_buf_red[BRIGHT_BG_ADDR]>0x07)	sim_buf_red[BRIGHT_BG_ADDR]=0x07;
// 	light_bg=sim_buf_red[BRIGHT_BG_ADDR]+1;		//±³¹âÁÁ¶Èaddr=====================================
// 	light_bg+=0X01;						//½µµÍ±³¹â×î´óÁÁ¶È
// 	//light_bg*=0X0A;										
// 	light_bg*=0X0a;
	
	light_bg=sim_buf_red[BRIGHT_BG_ADDR]+0x01;   //±³¹âÁÁ¶ÈÃüÁî0~15   ===================================
	if(light_bg>0x10)	light_bg=0x10;
	if(light_bg<0x07) light_bg=0x07;		
	light_bg*=0X03;				
...
// 	if(rtc_error_flag==0x01)//ÏÔÊ¾  Ğ£Ê±Ê§°Ü
// 	{
// 	g_wShowVersionCnt = 0;
// 	
...
// 		
	g_wShowVersionCnt = 0;
	ShowVersion(); 								// ²¥·Å½ÚÄ¿Ö®Ç°ÏÔÊ¾°æ±¾ºÅºÍ·¢²¼ÈÕÆÚ
// 	ledplay_denglong();
// 	delay(6000000);	
// 	while(1);
				...
				switch(test_Pattern)
				{
					...
					case 3:			//ºáÌõÏÂÒÆ
					{
						...
						while(n_time_paly)
						{
							...
							{
								for(m2=0;m2<hang_shu/8;m2++)		//Ã¿¸ô8ĞĞ
								{
...
// 										}
												...
												//memset(leddata(m+m2*8), 0xff, screen_len);
...
// 							ledplay_denglong();	
	...
	pro_c=0;
	mm_set_en=1;						//ÔÊĞíÉèÖÃWIFIÃÜÂë
	r1=RTC_GetCounter();		//¶Á³örtc¼ÆÊıÆ÷ÖĞµÄÃëÖµ					
	while(1)								//ÏÔÊ¾Ñ­»·
		{
			if(mm_set_en)
			{
					r2=RTC_GetCounter();//¶Á³örtc¼ÆÊıÆ÷ÖĞµÄÃëÖµ
					if((r2-r1)>60)	mm_set_en=0;	//³¬¹ı40Ãëºó¹Ø±ÕÃÜÂëÉè¶¨
					delay(20000);	
			}
	...
	/********************end*****************/
			...
			{	  //µ±Ç°½ÚÄ¿½áÊø±êÖ¾
					...
					if(light_bg<0X30)		//0X5a×îµÍÖµÎªÈ¡Ïû±³¹â¹¦ÄÜ
						if(j==light_bg)																				//light_bg 
						{
							j=0;
							ledplay_denglongHei(0xff);
							delay(40);
						}
					 datadeal();			  //Êı¾İ´¦Àí  
				if(scr_info.s_t){
				...
				if(pro_end1)							
				{
						while(1)
						{
								...
								if(!time_onoff)	
								{
									...
									memset(&leddata[0x16*SCREEN_LEN], 0x00, SCREEN_LEN);
									memset(&leddata[0x17*SCREEN_LEN], 0x00, SCREEN_LEN);
...
// 										else ii=0;	
								...
								j++;
								if(light_bg<0X5a)	//×îµÍÖµÎªÈ¡Ïû±³¹â¹¦ÄÜ
								if(j==light_bg)																				//light_bg 
								{
									j=0;
									ledplay_denglongHei(0xff);
