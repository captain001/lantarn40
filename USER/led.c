...
static void LEDShowVersion(u8 index, u8 n, u8 pos)
{
	u16 i,j;
	for(j=0;j<16;j++)
	  for(i = 0; i < n; i++){
		//leddata[(j + 1) * screen_len - 8 + pos+i] = ucaShowVersion[index+i][j];	
			leddata[(j + 1) * screen_len + pos+i] = ucaShowVersion[index+i][j];
 	  }  
// 	GREEN_BEGIN
// 		for(j=0;j<16;j++)
// 		  for(i = 0; i < n; i++){
...
// 	LEDShowModifyDate(MODIFYDATE/0X1000%0x10, 1, 2);
// 	LEDShowModifyDate(MODIFYDATE/0X100%0x10, 1, 3);
// 	//LEDShowModifyDate(10, 1, 5);//显示‘.’
// 	LEDShowModifyDate(MODIFYDATE/0X10%0x10, 1, 4);
// 	LEDShowModifyDate(MODIFYDATE%0x10, 1, 5);
// 	ledplay_denglong();
// 	delay(88000000);	
// 	while(1);
...
// 		
// 		flash_read_write(1);
// 		if(sim_buf_red[0x38] == 0xa0)  scr_info.of=0xa1;		//关机
	
			scr_onoff = 0;
		
// 		flash_read_write(1);
// 		if(sim_buf_red[0xa0] == 0xa0) 
// 		{
// 			scr_onoff=0xa1;		//关机
// 			flash_read_write(1);    
//  			sim_buf_red[0xa0] = 0xff;				//下次上电  自动开机
//  			flash_read_write(0);	
// 		}
...
void fun_time_asc(unsigned char *p)
{
	...
	for(i=0;i<20;i++)				//行
	{
		...
		{
			leddata[(i+2)*screen_len+j]<<=1;
			k=leddata[(i+2)*screen_len+j+1]&0x80;
			k>>=7;
			leddata[(i+2)*screen_len+j]|=k;
		}
		leddata[(i+2)*screen_len+screen_len-1]<<=1;
		temp<<=K;
		temp&=0x8000;
		temp>>=15;
		leddata[(i+2)*screen_len+screen_len-1]|=temp;
	}
	K++;
...
time_sw: //定时开关机
 ...
 set_wifi_key:                   //设置Wi-Fi热点密码
		if(mm_set_en) {								//上电60S内 允许设置WIFI密码
			flash_read_write(1);            
			i=ur();
								 //存储密码长度小于19字节且大于8个字节
			if(i>=8&&i<20)
